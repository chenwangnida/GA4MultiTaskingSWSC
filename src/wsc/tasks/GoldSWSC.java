...
public class GoldSWSC extends Task{
	@Override
	public double calculateSequenceFitness(Service[] sequence, WSCInitializer init, EvolutionState state, SequenceVectorIndividual ind2) {
		// generate DAG corresponding to vector
		InitialWSCPool.getServiceCandidates().clear();

		List<Service> relevantList = new ArrayList<Service>();

		for (Service s : sequence) {
			relevantList.add(s);
		}
		ind2.setRelevantList(relevantList);
		...
		double f = init.eval.calculateFitness(ind2);
